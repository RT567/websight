...
- **JS**: `shut()` adds `.shut` to body for 120ms. `blink()` calls it and reschedules itself 5–14s later, 15% chance of a quick double blink 250ms later; first blink at 1.5s. A `pointerdown` anywhere on the page also calls `shut()`, so clicking/tapping blinks (added 2026-09-02). `#name` is a fixed white "web sight" label in the top-left (22px sans-serif, pointer-events none), sitting under the lids so it blinks away with everything else (added 2026-09-02). Later the same day the fly was doubled (40×28px, offscreen start/end at -80px, path width W+160) and the `#err` box doubled (36px font, 24/36px padding, 12px yellow border). The bulb hover tooltip `#tip` was also doubled to 28px. `fly()` animates `#fly` with `requestAnimationFrame` across the viewport at a random height with two summed sine wobbles, shows `#err` for the duration, reschedules 15–40s later; first at 6s.
